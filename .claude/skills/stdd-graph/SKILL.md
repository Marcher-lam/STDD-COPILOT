...
│                                                                          │
│   ┌──────────────────┐   ┌──────────────────┐   ┌──────────────────┐   │
│   │  Dynamic Router  │   │  Graph Executor   │   │  Graph Cache     │   │
│   │  意图自适应拓扑   │   │  生命周期执行引擎  │   │  幂等断点缓存    │   │
│   └────────┬─────────┘   └────────┬─────────┘   └────────┬─────────┘   │
│            │                      │                       │             │
│            └──────────────────────┼───────────────────────┘             │
│                                   │                                      │
│   ┌──────────────────┐   ┌────────┴─────────┐   ┌──────────────────┐   │
│   │ Evidence Capture  │   │ Error Propagator │   │ Heterogeneous    │   │
│   │ 错误证据采集器    │   │ 多跳向上传播引擎  │   │ Adapter          │   │
│   └──────────────────┘   └──────────────────┘   │ 异构引擎适配层   │   │
│                                                  └────────┬─────────┘   │
│   ┌──────────────────┐                           │                      │
│   │ Parallel Executor│◄──────────────────────────┘                      │
...
当 DAG 节点执行失败时：
1. **EvidenceCapture** 截取结构化错误快照（错误类型、输入快照、阶段信息）
2. **ErrorPropagator** 沿依赖链向上多跳传播，寻找"决策点"（planning/gate/扇出）
3. 找到决策点后注入完整证据链，触发回炉重造
4. 未找到决策点则熔断，输出结构化失败报告

### 异构算力横向拓扑（抓手三）

22 个 AI 引擎（4 Tier 分层）统一调度：
1. **HeterogeneousAdapter** 按 Skill 兼容性映射引擎（full/partial/basic）
2. **ParallelExecutor** 将 DAG 拓扑分层，同层节点分配不同引擎并行执行
3. 失败时自动 Tier 降级重试
4. 并行组策略支持 all/any/race
